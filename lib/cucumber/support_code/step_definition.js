function StepDefinition(pattern, options, code) {
  ...
  ATTACH_DEFAULT_MIME_TYPE = 'text/plain';
  ATTACH_MISSING_MIME_TYPE = 'attach() expects a mimeType';
  ATTACH_MISSING_CALLBACK_FOR_READABLE_STREAM = 'attach() expects a callback when data is a readable stream';

  function isReadableStream(value) {
    return value && typeof value === 'object' && typeof value.pipe === 'function';
  }

  function attach(data, mimeType, callback) {
    if (isReadableStream(data)) {
      if (!mimeType)
        throw Error(ATTACH_MISSING_MIME_TYPE_ARGUMENT);
      if (!callback)
        throw Error(ATTACH_MISSING_CALLBACK_ARGUMENT_FOR_STREAM_READABLE);
      ...
      data.on('end', function () {
        attachments.push(Cucumber.Runtime.Attachment({
      ...
      if (!mimeType) {
        mimeType = ATTACH_DEFAULT_MIME_TYPE;
      }
